...
func main() {
	...
	http.HandleFunc("/callb", func(w http.ResponseWriter, r *http.Request) {
		start := time.Now()

		_, exists := os.LookupEnv("SELF_DELAY")

		if exists {
			//adding a fixed delay - akin to going down a bad code path
		...
		fmt.Fprintf(w, "Calling Dependency Endpoint\n")
		//just to check if prom is capturing the correct numbers
		incrementCounter()
		fmt.Fprintf(w, strconv.Itoa(counter))
		opsProcessed.WithLabelValues(r.Method, r.RequestURI).Inc()
	...
	log.Println("Listening on localhost:8090")
